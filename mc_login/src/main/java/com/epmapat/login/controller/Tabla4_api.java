...
import com.epmapat.login.service.Tabla4_ser;
...
@CrossOrigin("*")
public class Tabla4_api {
    @Autowired
    private Tabla4_ser tab4Servicio;
    ...
    public ResponseEntity<Tabla4> getByIdtabla4(@PathVariable Long idtabla4) {
        Tabla4 x = tab4Servicio.findById(idtabla4).orElseThrow();
    ...
    public ResponseEntity<Tabla4> update(@PathVariable Long idtabla4, @RequestBody Tabla4 x) {
        Tabla4 y = tab4Servicio.findById(idtabla4).orElseThrow();
